...
# Financial Agent MCP Central Architecture Startup Script
# Implements the exact architecture from the provided diagram:
# UI -> MCP Server -> Landing AI, Google API Inkeep, LLM -> Pathway -> Response

echo "🚀 Starting Financial Agent MCP Central Architecture"
echo "=================================================="
...
check_port 8000 || exit 1  # MCP Central Server (main orchestrator)
...
# Start LandingAI MCP Server (Step 2: MCP Server -> Landing AI)
echo -e "${BLUE}🤖 Starting LandingAI MCP Server (Step 2)...${NC}"
...
# Start MCP Central Server (Main Orchestrator)
echo -e "${BLUE}🎯 Starting MCP Central Server (Main Orchestrator)...${NC}"
cd /home/abhin/Financial-Agent-mcp
source venv/bin/activate
...
echo "=================================================="
echo ""
echo -e "${PURPLE}🏗️  ARCHITECTURE IMPLEMENTATION (from diagram):${NC}"
echo -e "   ${BLUE}UI${NC} -> ${GREEN}MCP Server${NC} -> ${YELLOW}Landing AI${NC} (Step 1-2)"
echo -e "   ${GREEN}MCP Server${NC} -> ${YELLOW}Landing AI${NC} -> ${GREEN}MCP Server${NC} (Step 2-3)"
echo -e "   ${GREEN}MCP Server${NC} -> ${PURPLE}LLM${NC} -> ${PURPLE}Pathway${NC} (Step 4-5)"
echo -e "   ${PURPLE}Pathway${NC} -> ${GREEN}MCP Server${NC} -> ${BLUE}UI${NC} (Step 6-7)"
echo ""
echo -e "${BLUE}🌐 Access Points:${NC}"
echo -e "   ${GREEN}MCP Central Server:    http://localhost:8000${NC}  (Main Orchestrator)"
...
echo ""
echo -e "${PURPLE}🔗 Architecture Flow (from diagram):${NC}"
echo -e "   ✅ Step 1: UI -> MCP Server (fast-apis)"
echo -e "   ✅ Step 2: MCP Server -> Landing AI (doc)"
echo -e "   ✅ Step 3: Landing AI -> MCP Server (action)"
echo -e "   ✅ Step 4: MCP Server -> LLM (XML)"
echo -e "   ✅ Step 5: LLM -> Pathway (refine it)"
echo -e "   ✅ Step 6: Pathway -> MCP Server (response)"
echo -e "   ✅ Step 7: MCP Server -> UI (Answers for display)"
echo -e "   ✅ Google API Inkeep Integration"
echo ""
echo -e "${YELLOW}🧪 To test the complete architecture:${NC}"
echo -e "   cd /home/abhin/Financial-Agent-mcp"
echo -e "   source venv/bin/activate"
echo -e "   python test_architecture_integration.py"
echo ""
echo -e "${YELLOW}🛑 To stop all services:${NC}"
echo -e "   kill $MCP_CENTRAL_PID $LANDINGAI_PID $INKEEP_PID $FRONTEND_PID"
echo ""

# Open MCP Central Server in browser (main orchestrator)
echo -e "${BLUE}🌐 Opening MCP Central Server (main orchestrator) in browser...${NC}"
...
# Monitor services
while true; do
    echo -e "${BLUE}$(date '+%H:%M:%S') - MCP Central Architecture running...${NC}"
    sleep 30
done
